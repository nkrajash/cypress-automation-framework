describe("Test contact-us form via Automation-test-store", () => {
    before( function() {
        //cypress code
        cy.viewport(550,750)
        cy.fixture("userdetails.json").as("user_dets")
    ...
    it("should be able to submit a successful submission via contact-us form ", () => {
        cy.get("@user_dets").then(user_dets => { 
            cy.get('#ContactUsFrm_first_name').type(user_dets.first_name)
            cy.get('#ContactUsFrm_email').type(user_dets.email)        
        })
